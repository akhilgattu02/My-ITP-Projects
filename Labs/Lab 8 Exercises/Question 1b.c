...
int main()
{
    ...
    int max_height = student_record[0].height;
    int min_height = student_record[0].height;
    for (int j = 0; j < 1000; j++)
    {
        if (student_record[j].height > max_height)
        {
            max_height = student_record[j].height;
        }
        if (student_record[j].height < min_height)
        {
            min_height = student_record[j].height;
        }
    }

    printf("Max height is: %d, Min height is: %d\n", max_height, min_height);
    return EXIT_SUCCESS;
}